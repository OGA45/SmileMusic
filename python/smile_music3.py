...
async def seek(ctx:discord.Interaction, seek:str):
    ...
    if player:
        ...
        try:
            if stream:
                player.original.seek(**ffmpeg_stream_options, seek_time=seek)
            else:
                player.original.seek(**ffmpeg_options, seek_time=seek)
            #player.original.wait_buffer()
        except:
            try:
                if stream:
                    player.seek(**ffmpeg_stream_options, seek_time=seek)
                else:
                    player.seek(**ffmpeg_options, seek_time=seek)
            except:
                await embed_response(ctx,"失敗","無効な形式です。",255,0,0,True)
                return
        finally:
            ctx.guild.voice_client.resume()
            await embed_response(ctx,"成功","シークしました。",0,255,0,True)
    else:
